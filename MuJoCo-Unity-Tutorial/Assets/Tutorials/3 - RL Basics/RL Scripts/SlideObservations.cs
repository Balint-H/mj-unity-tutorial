...
public class SlideObservations : SensorComponent {
  ...
  public List<float> Observations =>
      slideSensor.Observations; // Exposed in case needed for visualisation
  ...
  private class SlideSensor : ISensor {
    ...
    float[] Velocities {
      get => new[] { slide.Velocity };
    } // Note that velocity is already in radians in the Plugin

    float[] Positions {
      get => new[] { slide.Configuration };
    } // We Configuration is scaled to degrees, even for slide joints...
