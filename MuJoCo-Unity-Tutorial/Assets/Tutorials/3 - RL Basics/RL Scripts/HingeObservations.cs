...
public class HingeObservations : SensorComponent {
  ...
  public List<float> Observations =>
      hingeSensor.Observations; // Exposed in case needed for visualisation
  ...
  private class HingeSensor : ISensor {
    ...
    float[] Velocities {
      get => new[] { hinge.Velocity };
    } // Note that velocity is already in radians in the Plugin

    float[]
        ContinuousHingePositions // We separate the angle into 2 components so it is a continuous and doesn't jump from 359 deg to 0 deg.
    {
      get
      {
    ...
    float[] Positions {
      get => ContinuousHingePositions;
    } // We combine the hinge and slider position
