...
public abstract class RewardProvider : MonoBehaviour {
  ...
  // Additional information on guaranteeing synchronisation via MjScene events will be discussed in a later tutorial.
...
public class CartpoleReward : RewardProvider {
  ...
  public override float Reward {
    get => GetStateReward() * Regularization();
  }
  ...
  float GetStateReward() {
    ...
    return
        upright * centered *
        smallVelocity; // Multiply, so we need to satisfy all three at the same time
  }

  // We want to use as little force as possible to get there.
  float Regularization() {
    float control = slideActuator.Control;
    var smallControl = Tolerance(control,
        bounds: (0f, 0f),
        margin: 1,
        valueAtMargin: 0,
        sigmoid: "quadratic");
    ...
    if (new[] { "cosine", "linear", "quadratic" }.Contains(sigmoid)) {
      if (!(0f <= valueAt1 && valueAt1 < 1f)) {
        throw new ArgumentException(
            $"`value_at_1` must be nonnegative and smaller than 1, got{valueAt1}");
      }
